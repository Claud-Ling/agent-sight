...
3. **T5b/T5c 多轮上下文差异**：CC 的 T5b prompt 以"基于你刚对 agentOS 的分析"开头，T5c 以"总结我们讨论的核心设计原则"开头——这些引用交互式 session 中的前序上下文。TM 的 `--once-json` 模式下每个 session 独立运行，没有对话历史。这是 **`--once-json` 模式的固有差异**，不是 prompt 差异。对进程行为的影响：CC 的 T5b/T5c 可能因上下文连续性产生更多工具调用（读文件确认之前讨论的内容）。

4. **结论可信度分级**：结构性结论（fork vs 线程池、进程树形态、资源占用模式）不受 prompt 差异影响，可信度高。定量结论（进程数、调用数绝对值）受上述因素微弱影响，但比 v1 实验显著改善。
...
TM 的资源占用是 CC 的 1/18（RSS）到 1/72（CPU），但进程数更多（26 vs 14）。资源效率与进程数反向——TM 的子进程都是极轻量级的 shell 命令。
