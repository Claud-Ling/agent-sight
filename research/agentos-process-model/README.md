...
> **复核结论（见 `report.md` §2）**：观测推翻了"每次工具调用 fork 子进程"的初始假设。Claude Code（Bun runtime）的子进程集中在启动 + 首次 shell 环境探测（前 ~20s），稳态期工具执行在 Bun worker 线程池内进程内完成，零新子进程。这直接影响下面三问的答案——尤其 agentOS 的隔离/记账单元应下沉到线程/syscall/工具调用粒度，而非进程粒度。
