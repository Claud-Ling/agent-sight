...
**这是本轮深度复核推翻上一版报告的最重要结论。** 上一版提出"Skeleton-Burst / Web 服务器 per-request fork"模型——认为每次 LLM 调用产生一批子进程、用完即弃。**统一事件时间线分析（D15/D16）证明这是错的。**
...
上一版把 `env + bash + locale-check + grep×4 + base64×6 + head + awk + sed×2 + cat×3` 解释为"Bash 工具调用触发的 6 路并行文件读取流水线"。证据不支持：
...
**根 vs 孤儿的修正**：上一版把"7 个 root_candidates"直接当成多根，措辞含糊。修正后的脚本（`process_tree_stats`）明确区分两类无父节点：
...
**修正：不存在 `node` 进程。** 上一版多处称"node runtime 长期常驻，7/7"，这是错的——7 个 session 的进程 comm 里**没有任何 `node`**。被误认为 node 的是 `claude.exe`（共 21 个实例），而它**不是常驻 runtime**：每个实例存活 1-7ms 就退出（见 §5）。唯一真正长寿的进程是 `claude` 自身（Bun 主进程）。
...
**修正 INFRA/ACTION 分类**：上一版按 comm 名硬编码（把 `claude` 和 `node` 都判为 INFRA）。修正后改用**实测存活时间**分类——p50>1s 才是 INFRA。结果：
...
**上一版称"exit_code=0 占比 >95%、未观测到失败、失败率 0%"，与数据不符。** 跨 7 个 session 共 17 次非零退出：`claude.exe=1`（×6，Bun worker 探测性退出）、`grep=1`（×10，grep 无匹配的正常退出码 1）、`git=1`（×1）。

这些"失败"多数是**预期的控制流信号**而非真错误——`grep` 无匹配返回 1 是正常语义（参见 [bestpractice_07-bash_strict_mode_pipes.md]）。但报告必须如实呈现：失败率是 ~8%，不是 0%。
...
**修正后的"认知↔行动"相位关系**：推理阶段（LLM 调用，3-30s）不创建子进程；但**行动阶段同样不创建子进程**——工具执行在 Bun 线程池内。子进程创建唯一集中在 (a) 启动自检、(b) 首次 shell 环境探测，这两件事都发生在 session 前 ~20s。

**对 agentOS 的启示**：上一版说"可利用推理阶段回收上一轮工具进程"——这个建议建立在错误前提上（稳态根本没有工具进程可回收）。修正后的启示是：agentOS 在推理阶段（LLM I/O 等待，数秒到数十秒）应让出 CPU——但让出的是**整个 agent 进程的 CPU 时间片**（因为稳态就是单进程），让其他 agent 使用。这是会话级（而非进程级）的 CPU 调度。
...
**缺父孤儿（base64）的机制**：每个 session 的 6 个 `base64` 进程，ppid 指向 5 个不同的、未被捕获的中间 pid（如 T4 的 302732/302735/...）。这些中间进程是 bash 管道里 `cmd | base64` 的左侧环节，在 eBPF 记录其 exec 前已退出（亚毫秒）。这是 eBPF 进程追踪的已知竞态盲区：极短命的管道中间进程可能在父子关系被记录前消失。修正后的脚本把它们明确标为 `missing_parent_orphans`，不再混入"多根"统计。
...
**修正后的模型：agent 进程模型更像"单进程事件循环 runtime + 固定 worker 线程池"（Bun/Node 自身的形态），而非 Web 服务器的 per-request fork。** 上一版的"Skeleton-Burst / per-request fork"判断被数据推翻——子进程不随 LLM/工具调用产生，而是集中在不可避免的启动+引导期；任务的认知↔行动主循环全部在单个长寿进程内闭合。
...
*Created: 2026-06-25 | Updated: 2026-06-25（深度复核第二轮：方法学修正 + 统一事件时间线 D15/D16，推翻 per-request-fork 模型，确立"单 runtime + 线程池 / 进程内工具执行"模型；修正 node→claude.exe、失败率 0%→~8%、根/孤儿区分三处事实错误）*
