...
- **版本化提醒策略**: 仓库通过 `.claude/periodic_jobs/ai_heartbeat/config/reminder_policy.json` 定义默认 reminder policy；当前 schema 只保留 `windows_popup_enabled`，不把 UI 偏好混进本地运行态。
...
- **提醒策略文件**: `.claude/periodic_jobs/ai_heartbeat/config/reminder_policy.json`，schema 只保留 `windows_popup_enabled`。
