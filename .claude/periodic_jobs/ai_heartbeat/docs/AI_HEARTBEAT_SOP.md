...
1. 先读取以下文件，再决定执行路径：
   - `AGENTS.md`
   - `.claude/periodic_jobs/ai_heartbeat/docs/KNOWLEDGE_BASE.md`
   - `.claude/periodic_jobs/ai_heartbeat/docs/PRD.md`
...
```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_preflight.py --command-spec
```
...
1. 先读取 `.claude/memory/OBSERVATIONS.md`。
...
```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_status_cli.py observer --status skipped --target-date <target_date>
```

   - 然后把 observer 视为 `skipped`，允许 `force both` 或默认双任务路径继续进入 reflector。
4. 如果当天条目不存在：
   - 按 `.claude/periodic_jobs/ai_heartbeat/docs/KNOWLEDGE_BASE.md` 与 `.claude/periodic_jobs/ai_heartbeat/docs/PRD.md` 定义的 observer 语义，扫描、过滤、归纳并写入 `.claude/memory/OBSERVATIONS.md`。
   - observer 只更新观测层，不要在这一步修改 `rules/`。
   - 成功后立即运行：

```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_status_cli.py observer --status success --target-date <target_date>
```
...
```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_status_cli.py observer --status failed --target-date <target_date> --error "<brief error>"
```
...
1. 读取 `.claude/memory/OBSERVATIONS.md` 与相关规则面。
2. 按 `.claude/periodic_jobs/ai_heartbeat/docs/KNOWLEDGE_BASE.md` 与 `.claude/periodic_jobs/ai_heartbeat/docs/PRD.md` 定义的 reflector 语义执行晋升、整理和 GC。
3. 成功后立即运行：

```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_status_cli.py reflector --status success --target-date <target_date>
```

4. 如果 reflector 失败：

```text
<PYTHON> .claude/periodic_jobs/ai_heartbeat/src/v0/heartbeat_status_cli.py reflector --status failed --target-date <target_date> --error "<brief error>"
```

## 写入工具注释（OBSERVATIONS.md）

向 `.claude/memory/OBSERVATIONS.md` 写入 observer 结果时，必须使用以下策略：
