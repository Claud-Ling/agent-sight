...
Related: [04_COMMUNICATION.md §文件交叉引用规范] [05_SKILLS_INDEX.md §如何添加你自己的 Skill]
...
**交叉引用。** 章节标题使用 `§` 前缀标记（如 `## §核心流程`），章节末尾添加 `Related:` 行连接到关联文件/章节。完整规范见 [04_COMMUNICATION.md §文件交叉引用规范]。
...
Related: [06_AXIOMS_INDEX.md §V. 信任与验证] [bestpractice_02-ai_programming_mindset.md §"70% 问题"的诊断与解决]

## §与现有 skill 的关系

写新 skill 之前，先读 `rules/05_SKILLS_INDEX.md` 确认没有重复。如果已有类似 skill，优先修改而非新建。

格式参考可以看`rules/skills/bestpractice_04-temporal_info_verification.md`（最佳实践类 skill 的范本）。注意这些只是格式参考，核心原则（结果确定性、enabling 而非 SOP）比格式更重要。

Related: [03_WORKSPACE.md §创建阈值] [05_SKILLS_INDEX.md §分类索引]
